...
describe ("Definition") do
  it("returns an empty array for storing word definitions") do
    testdictionary = Definition.new({ :text => "a flying death machine", :id => nil, :word_id => 1 })
...
describe ("update") do
  it("updates word definitions") do
    testdictionary = Definition.new({ :text => "a flying death machine", :id => 1, :word_id => 1 })
...
describe("#delete") do
  it("Deletes a definition by id") do
    testdictionary = Definition.new({ :text => "a flying death machine", :id => nil, :word_id => 1 })
    testdictionary.save()
    testdictionary2 = Definition.new({ :text => "not a flying death machine", :id => nil, :word_id => 2 })
...
describe(".clear") do
  it("clears definitions") do
    testword = Definition.new({ :text => "a flying death machine", :id => nil, :word_id => 1 })
    Definition.clear()
    expect(Definition.all).to(eq([]))
  end
end
